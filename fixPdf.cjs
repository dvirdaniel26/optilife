...
let content = fs.readFileSync('src/pages/ActionPlanPage.jsx', 'utf8');

// 1. Unhide header
content = content.replace(
  /\`flex-col sm:flex-row justify-between sm:items-end gap-4 mb-8 border-b border-slate-100 pb-4 \$\{isGeneratingPdf \? 'hidden' : 'flex'\}\`/,
  `"flex-col sm:flex-row justify-between sm:items-end gap-4 mb-8 border-b border-slate-100 pb-4 flex print:hidden"`
);

// 2. Unhide tabs
content = content.replace(
  /\`flex gap-1 bg-slate-100\/70 p-1 rounded-2xl w-full sm:w-fit print:hidden \$\{isGeneratingPdf \? 'hidden' : ''\}\`/,
  `"flex gap-1 bg-slate-100/70 p-1 rounded-2xl w-full sm:w-fit print:hidden"`
);

// 3. Fix nutrition tab visibility
content = content.replace(/\(activeTab === 'nutrition' \|\| isGeneratingPdf\)/g, "(activeTab === 'nutrition')");
content = content.replace(
  /className=\{activeTab === 'nutrition' \|\| isGeneratingPdf \? 'space-y-5 print:block' : 'hidden print:block'\}/,
  `className="space-y-5 print:block"`
);

// 4. Fix fitness tab visibility
content = content.replace(/\(activeTab === 'fitness' \|\| isGeneratingPdf\)/g, "(activeTab === 'fitness')");
content = content.replace(
  /className=\{activeTab === 'fitness' \|\| isGeneratingPdf \? 'space-y-5 print:block' : 'hidden print:block'\}/,
  `className="space-y-5 print:block"`
);

// 5. Change target element
content = content.replace(
...
// 6. After PDF generation, hide it again
content = content.replace(
...
// 7. Inject the template before closing main
const template = `
        {/* ── HIDDEN PDF TEMPLATE ── */}
...
content = content.replace(/\s*<\/div>\s*<\/main>/, template);

fs.writeFileSync('src/pages/ActionPlanPage.jsx', content);
console.log('ActionPlanPage.jsx updated successfully.');
